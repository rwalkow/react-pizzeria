    ...
    <Form onSubmit={validate(handleSubmit)}>
      <Row className="mb-2 mt-2 justify-content-md-center">
        <Col sm={1} className="ps-3 fw-bold">
          <Form.Label className="pt-2 align-baseline" htmlFor="status">
            Status:
          </Form.Label>
          </Col>
        <Col sm={4}>
          <Form.Select {...register("status", { required: true, })}
            aria-label="Select category..." id="status" name="status" value={status} onChange={e => onChangeCategory(e.target.value)}>
        ...
        <Col sm={4}>
          <Row className="row justify-content-start g-0">
            ...
            <Col sm={2} className="position-relative">
              <Form.Control aria-label="per"
                className="form-control-plaintext text-center fs-2 position-absolute top-50 start-50 translate-middle bg-white"
                readOnly type="text" id="peopleAmountStr" name="peopleAmountStr" value="/"/>
            </Col>

            <Col sm={5}>
              <Form.Control
                {...register("peopleAmountMax", { required: true, min: 1, })}
      ...
      <ConditionRenderer condition={status === 'busy'}>
        <Row className="mb-2 mt-2 justify-content-md-center">
          ...
          <Col sm={4}>
            {status === 'busy' && <InputGroup className="mb-3">
                <InputGroup.Text id="basic-addon1">$</InputGroup.Text>
                <Form.Control {...register("bill", { required: true, min: 1, })}
                  aria-label="Bill" type="number" id="bill" name="bill" value={bill} onChange={e => setBill(e.target.value)}/>
              </InputGroup>
            }
